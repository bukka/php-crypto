...
PHP_CRYPTO_METHOD(Rand, loadFile)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "s|l", &path, &path_len, &max_bytes) == FAILURE) {
		return;
	}
...
PHP_CRYPTO_METHOD(Rand, writeFile)
{
	...
	if (bytes_written < 0) {
		PHP_CRYPTO_THROW_RAND_EXCEPTION(0, "The bytes written were generated without appropriate seed");
	} else {
